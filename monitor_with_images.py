...
def run_app(playwright):
    ...
    def download_by_fid(fid):
        source = "209678993"
        try:
            meta = page.evaluate(f"""
            () => {{
                return new Promise((resolve) => {{
                    window.__meta_cb = function(data) {{ resolve(data); }};
                    let s = document.createElement('script');
                    s.src = 'https://upload.api.weibo.com/2/mss/meta_query.json?source={source}&fid={fid}&replace=false&callback=__meta_cb';
            ...
            ext = meta.get("extension", "jpg") if meta else "jpg"
            orig_name = meta.get("filename", f"{fid}.{ext}") if meta else f"{fid}.{ext}"
            filename = f"img_{fid}_{orig_name}"
            ...
            download_url = f"https://upload.api.weibo.com/2/mss/msget?fid={fid}&source={source}&imageType=origin&ts={ts}"
            
            with page.expect_response(
                lambda resp: str(fid) in resp.url and "msget" in resp.url,
                timeout=15000
            ) as response_info:
                page.evaluate(f"""() => {{
            ...
            logging.error(f"Failed to download fid {fid}: {e}")
        return None
        ...
        with open(output_file, "a", encoding="utf-8") as f:
            for msg in sorted_msgs:
                ...
                if fids:
                    media_paths = []
                    for fid in fids:
                        downloaded_path = download_by_fid(fid)
                        if downloaded_path:
